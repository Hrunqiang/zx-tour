<?php
class CommonV2Action extends Action{
...
// 		echo $Reques_uri;die;
		...
		if($ac=='list'){
			...
			$list	= $Common->getList($table_cfg['tables'],$list_field,$start,$pagenum,"$where",$tables[0].".ctime desc");
...
// 			$info['attr'] = $Common->unattr($info['attr']);
			...
			if($_POST){
				$rs	= $Common->add($table_cfg['tables'], $_POST);
				if($rs){
			...
			foreach ($_POST as $k=>$v){
				if($k == "id"){
					...
					if(is_array($id)){
						...
						$id	= implode(",", $id);
						$where  .= " o.orderid in (".$id.") and" ;
					}else{
						$where  .= " o.orderid = '$id' and" ;
					}
				}else{
			...
			if($where) $where = rtrim($where,"and");
			EnrollAction::export($where);
			die;
// 			$Reques_uri	= $Reques_uri."&ac=list";
// 			$Reques_uri	= preg_replace("/&?id=?[\d]{0,}/", "", $Reques_uri);
			//$rs	= $Common->del($tables[0], $id);
// 			if($rs){
