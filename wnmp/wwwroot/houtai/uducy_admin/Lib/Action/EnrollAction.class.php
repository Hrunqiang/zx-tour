				...
				"tables"=>array(
						...
						'mls_baomingdan'=>array(
								...
								"field"=>array(
										...
										'match'	=>array('title'=>'赛事','isabm'=>true,'islist'=>true,'type'=>'text'),
								)
						),
				),
				's'		=>'SdkNumData',
...
// 		var_dump($list);
	...
	function exportexcel($data=array(),$title=array(),$filename='report'){
		...
		header("Expires: 0");
		if(empty($title)){
			$title = array("赛事","订单号","姓名","性别","手机","下单平台","订单状态","价格","支付订单号","支付信息","邮箱","姓名拼音-姓","姓名拼音-名","国籍","身份证号","护照号","护照签发日期","护照有效日期","生日","省","市","地区","地址","邮编","是否参加全马","最好成绩","最好成绩赛事","预期成绩","装备尺码","紧急联系人","与紧急联系人关系","紧急联系人邮箱","紧急联系人电话","创建时间","订单最后更新时间","注释","商品详情");
		}
		//导出xls 开始
		if (!empty($title)){
		...
		if (!empty($data)){
			...
			foreach($data as $key=>$val){
				$list = $orderinfoM->getorderinfo($data[$key]['orderid']);
				...
				foreach ($list as $lk =>$lv){
					$matchinfo.=$lv['goodscls']."+";
				}
				$matchinfo = rtrim($matchinfo,"+");
				$data[$key]["info"] =iconv("UTF-8", "GB2312", $matchinfo) ;
