...
class UserAuthModel extends Model {
	protected $trueTableName = 'mls_admin_user';
	protected $connection = 'DB_CONFIG1';

	public function auth($username, $password) {
		$username = addslashes ( $username );
		$password = md5 ( C ( 'USER_SALT' ) . addslashes ( $password ) );
		$rs = $this->field ( "id,password,is_admin,right_action" )->where ( "`user_name`='{$username}'" )->find ();
	...
	public function member_list(){
		$userlistdb	= $this->field("user_name,is_admin,lastip,last_login")->order("is_admin desc")->select();
		return $userlistdb;
	}
	public function member_add($username,$pwd,$phone,$phonetype){
