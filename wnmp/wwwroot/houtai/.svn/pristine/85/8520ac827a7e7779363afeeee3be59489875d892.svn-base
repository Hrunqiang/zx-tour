...
// 		$rs	= $this->field("count(id) as c")->where("cls = '$cls'")->find();
	...
	public function del($table,$id,$fieldid="id",$delfield,$utime="utime"){
		if(empty($table)||empty($id))return false;
		$rs	= $this->table($table)->where("$fieldid in( $id )")->data(array($delfield=>'1',$utime=>Date("Y-m-d H:i:s")))->save();
		return $rs;
	}
	
	public function updatestatus($table,$id,$fieldid="id",$status,$delfield,$utime="utime"){
		if(empty($table)||empty($id))return false;
		$rs	= $this->table($table)->where("$fieldid in( $id )")->data(array($delfield=>$status,$utime=>Date("Y-m-d H:i:s")))->save();
		return $rs;
	}
