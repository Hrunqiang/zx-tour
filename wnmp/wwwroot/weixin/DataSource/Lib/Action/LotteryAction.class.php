...
// 		}
	...
	public function bolin(){
		...
		$M = new DmCXModel;
		$res = $M->select_score_table($code,"zx_hd_bolin");
		if($res){
			//echo $res['splits']."<br>";
			echo json_encode(array("error"=>0,"msg"=>"ok","data"=>$res));
		}else{
			echo json_encode(array("error"=>1,"msg"=>"error0011","data"=>""));
		}
	}
	...
	public function chicago(){
		...
		$M = new DmCXModel;
		$res = $M->select_score_table($code,"zx_hd_chicago");
		if($res){
			//echo $res['splits']."<br>";
			$finishTimeNet = $res['time_finish'];
			$count = $M->select_chicago_score_table_rank($finishTimeNet);
			$count = $count!==false?$count+1:"0";
			echo json_encode(array("error"=>0,"msg"=>"ok","data"=>$res,"rank"=>$count));
		}else{
			echo json_encode(array("error"=>1,"msg"=>"error0011","data"=>""));
		}
	}
