...
class EnrollAction extends Action {
	...
	public function index() {
		...
		if($matchid==287){
			$t = htmlspecialchars(addslashes(trim($_REQUEST['t'])));
			if($t!=md5($matchid."zxtour")){
				redirect("/");
			}			 
		}
		
		if($matchid==247){
			if($date<="2016-09-21 12:00"){
				$t = htmlspecialchars(addslashes(trim($_REQUEST['t'])));
				if($t!=md5($matchid."zxtour")){
					redirect("/");
				}
			}else if($date>"2016-09-21 12:00" && $date<"2016-09-21 13:00"){
				redirect("/");
			}
		}
		
// 		if($matchid==350){
// 			if($date<="2016-11-07 16:00:00"){
// 				$t = htmlspecialchars(addslashes(trim($_REQUEST['t'])));
// 				if($t!=md5($matchid."zxtour")){
// 					redirect("/");
// 				}
...
// 		}
	...
	public function createorder(){		
		...
		$orderData['platform'] = session("SESSION_ZX_PLATFORM")?session("SESSION_ZX_PLATFORM"):"zx-tour";
				...
				foreach ($orderConfig as $key=>$value){
					...
					if($goodsIdList){
							...
							if($goodsinfo){
								...
								if($issingle && $key=="meal"){
									...
									if($orderinfoM->addOrders($data)){	
										...
										header("Content-type:text/html;charset=utf-8;");
										$msg = '<h2><a href="/Matchinfo?m_id='.$matchid.'" >返回赛事首页</a></h2><script>alert("下单失败，请重试！！！")</script>';
										die($msg);
									}
								}
							}else{
								...
								header("Content-type:text/html;charset=utf-8;");
								$msg = '<h2><a href="/Matchinfo?m_id='.$matchid.'" >返回赛事首页</a></h2><script>alert("名额已满！您可以尝试其它赛程！")</script>';
								die($msg);
							}
						}
					}else{
						if($key=="course"){
							...
							header("Content-type:text/html;charset=utf-8;");
							$msg = '<h2><a href="/Matchinfo?m_id='.$matchid.'" >返回赛事首页</a></h2><script>alert("请选择您要报名的赛程！！")</script>';
							die($msg);
						}
					}
				}
