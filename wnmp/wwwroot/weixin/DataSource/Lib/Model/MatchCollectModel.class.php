...
class MatchCollectModel extends Model{
	...
	public function getMycollectMatchV2($uid){
		if(empty($uid)) return false;
		$sql = "SELECT m.id,m_name,m_GameTime,m_untilTime,m_releaseTime,m_offineTime,m_city,m_Mtypes,m_ptype,m_sign,m_enterMode,m_placesleft,m_places,m_enterurl,m_state,m_img,m_order,m_country,m_area,m_signuptime,m_mtypes_class,m_des FROM `zx_tb_collect` c LEFT JOIN `zx_tb_matchs` m ON m.id= c.match_id WHERE c.uid= $uid";
		return $this->cache(true,$this->cacheTime)->query($sql);
	}
