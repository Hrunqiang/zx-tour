...
var weui = {
	...
	Loading : {
		show : function(msg) {
			...
			weui_loading.show().find(".weui_toast_content").html(text);
        ...
        success: function(data){
            ...
            if(data.error == 0 ){           
                ...
                for(var key in list){
                	...
                	var minge='';
                	if(list[key].m_state!=0 || list[key].m_GameTime <=now || list[key].m_releaseTime >now || list[key].m_untilTime<now || list[key].m_offineTime <now ||list[key].m_placesleft==0){
                	...
                	htmlStr+='<div class="weui_media_hd"><img class="weui_media_appmsg_thumb" src-img="'+list[key].m_img+'" alt=""></div><div class="weui_media_bd"><h4 class="weui_media_title flex"><span class="S_overhiden">'+list[key].m_name+'</span>'+minge+'</h4><p class="weui_media_desc">'+list[key].m_des+'</p><p class="weui_media_desc weui_media_desc_padding">'+m_gametime+' '+m_mtypes+'</p></div></a>'           	
                }
