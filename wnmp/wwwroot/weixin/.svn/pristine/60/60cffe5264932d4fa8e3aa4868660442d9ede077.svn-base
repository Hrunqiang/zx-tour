 ...
 */
class HdPicAction extends Action {
	...
	public function piclist(){
		$m = new HdPicModel();
		$data  = $m->where("state=1")->order("zan desc")->select();
		if($data){
	...
	public function upload(){
		...
		$date = Date("Y-m-d H:i:s");
		if($date>="2016-08-19 18:00"){
			echo '<script type="text/javascript">alert("活动已结束！");window.location.href = "/h5/pic/list.html";</script>';
			die;
		}
		if($this->isPost()){
			...
			if(empty($data['imgdata'])||empty($data['msg'])){
				echo '<script type="text/javascript">alert("请上传图片和感言！");window.location.href = "/h5/pic/upload.html";</script>';
			}else{
				...
				if($uid){			
					...
					if($res){
						echo '<script type="text/javascript">alert("上传成功！");window.location.href = "/h5/pic/list.html";</script>';
					}else{
						echo '<script type="text/javascript">alert("可能您已经上传过照片了！");window.location.href = "/h5/pic/upload.html";</script>';
					}
				}else{
					echo '<script type="text/javascript">alert("请先验证你的手机号！");window.location.href = "/h5/pic/";</script>';
				}
			}
		}
	}
	...
	public function auth(){
		...
		if($this->isPost()){
			...
			if(!phoneCheck($phone)){
				echo '<script type="text/javascript">alert("手机号格式不对！");window.location.href = "/h5/pic";</script>';
			}else{
				...
				if(!empty($verifyCode)&&$verifyCode == $memcache ->get("ZX_TMP_PHONE_CODE_".$phone)){
					...
					if($u_id){
						...
						redirect ( "/h5/pic/upload.html" );
					}else{
						echo '<script type="text/javascript">alert("系统错误！");window.location.href = "/h5/pic";</script>';
					}
				}else{
					echo '<script type="text/javascript">alert("验证码错误！");window.location.href = "/h5/pic";</script>';
				};
			}
		}
