...
class DmCXModel extends Model{
	...
	public function select_chicago_score_table_rank($score){
		if(empty($score)) return false;
		$where = " time_finish<'$score' and time_finish is not null and time_finish!=''";
		return $this->cache(true,$this->cacheTime)->table("zx_hd_chicago")->where($where)->count();
