...
class UserOrdersModel extends Model{
	...
	public function getorderinfo($orderid,$where){
		...
		$sql = "SELECT o.orderid,o.payprice,o.pay_deadline,o.paystats,o.matchid,m.m_name,m.m_ptype,o.matchid,o.couponid,o.discount FROM zx_tb_orders o LEFT JOIN zx_tb_matchs  m ON o.matchid = m.id WHERE o.orderid= '$orderid' $where ";
	...
	public function getUseforOrder($uid,$matchid){
		if(empty($uid) || empty($matchid)) return false;	
		return  $this->where("uid = '$uid' and matchid='$matchid' and ( paystats=5 or paystats=1 )")->order("paystats asc")->find();
	} 
	
	public function getUseforOrderV2($uid,$matchid){
		if(empty($uid) || empty($matchid)) return false;
		return  $this->where("uid = '$uid' and matchid='$matchid' and ( paystats=5 or paystats=1 )")->order("paystats asc")->find();
	}
	...
	public function getorderinfo_notice($orderid){
		if(empty($orderid)) return false;
		$sql = "SELECT o.uid,o.payprice,u.`state`,o.orderid,o.orderprice,u.phone,o.paystats,o.payorderid,o.payinfo,o.platform,o.discount,o.official_notes,b.name,b.phone,b.e_mail,b.pass_code,m.m_name,m.m_ptype,m.m_notice_mail,o.ctime,o.utime FROM zx_tb_orders o LEFT JOIN zx_tb_userinfo b ON o.uid = b.uid LEFT JOIN zx_tb_user u ON o.uid = u.id LEFT JOIN zx_tb_matchs  m ON o.matchid = m.id WHERE o.orderid = '$orderid'";
		return $this->query($sql);
	}
	...
	public function getorderListByUid($uid){
		...
		$sql ="SELECT o.orderid,o.pay_deadline,m.m_untilTime,m.m_offineTime,m.m_state,m.m_placesleft,m.m_icon,m.m_name,m.id as mid,m.m_img,o.payprice,o.discount,o.paystats,m.m_GameTime FROM `zx_tb_orders` o LEFT JOIN `zx_tb_matchs` m ON o.matchid= m.id WHERE o.uid = $uid and o.paystats>=0 order by o.ctime desc";	
		return $this->query($sql);
	}
...
// 						"number"=>$number,
				...
				if(!$rs){
					...
					$couponM->useCoupon($order['couponid'],$out_trade_no);
					return true;
				}
			}
		}else{
	 ...
	 * @return boolean
	 */
	public function delOrder($orderid,$matchid){
		$this->startTrans();
		$del = $this->where("orderid='$orderid'")->data(array("paystats"=>3))->save();
		if($del){
			$matchM =  new  MatchInfoModel();
			if($matchM->updateMatchLeft($matchid,1)){
				$orderinfoM = new OrdersInfoModel();
				$orderinfoM->startTrans();
				if(false===$orderinfoM->releaseOrders($orderid)){
					$this->rollback();
					$orderinfoM->rollback();
					return false;
				}else{
	...
	public function getorderlist($where){
		...
		$sql = "SELECT o.orderid,m.m_name,m_ptype,u.state,i.e_mail,i.pass_code,i.surname,i.en_name,i.phone
FROM `zx_tb_orders` o
LEFT JOIN  `zx_tb_user` u ON o.uid = u.id
LEFT JOIN  `zx_tb_userinfo` i ON  o.uid  = i.uid
