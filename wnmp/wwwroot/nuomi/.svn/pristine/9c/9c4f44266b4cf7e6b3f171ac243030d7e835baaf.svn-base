...
class MailModel{
	protected $mail_config = array(
		...
		"Username"   =>"notice@zx-tour.com",
		"Password"   =>"Zhixingheyi1",
		"AddReplyTomail"=> "notice@zx-tour.com",//回复地址
		"AddReplyToname" =>"知行合逸客服",
		"From"=> "notice@zx-tour.com",
		"FromName"   =>"知行合逸客服"
		);
	...
	public function send($email,$title,$content){
		...
		try {
			...
			$mail->Send();
 			...
 			//echo Date("Y-m-d H:i:s")."邮件发送失败：".$e->errorMessage();
			@mwtlog("mail_send_data","mail:".$email."\t ".Date("Y-m-d H:i:s")."title".$title."\t content:".$content,true);
			return false;
		}	
		return true;
	}
