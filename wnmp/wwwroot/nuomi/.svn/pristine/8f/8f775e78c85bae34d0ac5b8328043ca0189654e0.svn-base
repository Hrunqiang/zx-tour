	...
	public function content(){
		...
		switch ($ac){
			...
			case 'list':
				...
				$isadv	= addslashes(intval($_GET['isadv']));
				if(!empty($cid)){
				$where	.= ' and a.cid='.$cid.' ';
				}
				if(isset($_GET['sign'])){
			...
			case 'add':
				if($_POST){
					...
					throw new Exception('类名称不能空!');
						...
						$data['n_color']		= htmlspecialchars(addslashes(trim($_POST['n_color'])));
						$data['orderid']		= htmlspecialchars(addslashes(trim($_POST['orderid'])));
						$data['n_img']			= htmlspecialchars(addslashes(trim($_POST['n_img'])));
						$data['n_imgsize']			= htmlspecialchars(addslashes(trim($_POST['n_imgsize'])));
						$data['n_url']			= addslashes(trim($_POST['n_url']));
						$data['n_from']			= addslashes(trim($_POST['n_from']));
						$data['n_des']			= htmlspecialchars(addslashes(trim($_POST['n_des'])));
						...
						$data['n_url_type']		= htmlspecialchars(addslashes(trim($_POST['n_url_type'])));
						$data['class']		= htmlspecialchars(addslashes(trim($_POST['class'])));
						$data['ctime']			= date("Y-m-d H:i:s");
						...
						$data['n_attr']			= trim($_POST['n_attr']);
						$data['n_attr']			= $ClassObj->enattr($data['n_attr']);
						$data['lastupdater']	= session('user');
						$data['blacklist']			= htmlspecialchars(addslashes(trim($_POST['blacklist'])));
						$data['isprivate']			= htmlspecialchars(addslashes(trim($_POST['isprivate'])));
						$data['channel']			= htmlspecialchars(addslashes(trim($_POST['channel'])));
						$data['parent_app']			= htmlspecialchars(addslashes(trim($_POST['parent_app'])))?htmlspecialchars(addslashes(trim($_POST['parent_app']))):NULL;
						$data['isadv']			= htmlspecialchars(addslashes(trim($_POST['isadv'])));
						$data['issetupadv']			= htmlspecialchars(addslashes(trim($_POST['issetupadv'])));
						$data['bundleid']			= addslashes(trim($_POST['bundleid']));
						$data['appstore_url']			= addslashes(trim($_POST['appstore_url']));
					}
					$rs	= $ClassInfObj->data($data)->add();
					if($rs){
						$blacklist = new IosSDKBlackListModel;
						$blacklist->updata_blacklist($rs, $data['blacklist']);
					}
					if($rs===false){
						throw new Exception('运行失败。!');
					}
					message('操作成功。','./?s=Classes&a=content&ac=add&cid='.$_POST['cid'].'&sname='.$sname);
				}
				$cid	= (empty($_GET['cid']))?0:$_GET['cid'];
				$select	= $ClassObj->dafenglei_select(0, 0, $cid);
			...
			case 'edit':
				if($_POST){
					if(empty($_POST['cid'])){
						...
						$data['blacklist']			= htmlspecialchars(addslashes(trim($_POST['blacklist'])));
						$data['isprivate']			= htmlspecialchars(addslashes(trim($_POST['isprivate'])));
						$data['parent_app']			= htmlspecialchars(addslashes(trim($_POST['parent_app'])));
						$data['lastupdater']	= session('user');
						$data['isadv']			= htmlspecialchars(addslashes(trim($_POST['isadv'])));
						$data['issetupadv']			= htmlspecialchars(addslashes(trim($_POST['issetupadv'])));
						$data['bundleid']			= addslashes(trim($_POST['bundleid']));
						...
						$blacklist = new IosSDKBlackListModel;
						$blacklist->updata_blacklist($id, $data['blacklist']);
						if(empty($id)){
				...
				$data['n_attr']		= $ClassObj->unattr($data['n_attr']);
				$data['n_des']   = htmlspecialchars_decode(stripslashes($data['n_des']));
				$data['n_url']   = stripslashes($data['n_url']);
				$data['n_from']   = stripslashes($data['n_from']);
