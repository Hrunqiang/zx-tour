...
class ClassInfoModel extends CoreModel {
	protected  $trueTableName	= "ios_news_class";
	protected  $ChirdTableName	= "ios_news";
	...
	public function getContent($sname,$lenght=10,$page=1,$field='',$where='',$order){
		...
		if(empty($field)){
			if('er_index_game'==$sname||'er_index_game_2'==$sname){
				...
				$field = "news.id,news.cid,news.n_title,news.n_img,news.n_url,news.n_from,news.sign,news.n_des,news.parent_app ";
			...
			}else if('quick_widgetadv'==$sname){
				$field	= 'news.id,news.cid,cls.classname,news.n_title,news.n_img,news.n_url,news.orderid,news.n_des,news.utime,news.n_imgsize,news.sign,news.advtype,news.isadv,news.n_url_type,news.n_content,news.n_imgsize2 ';
			}
			else{
				$field	= 'news.id,news.cid,cls.classname,news.n_title,news.n_img,news.n_url,news.orderid,news.n_des,news.utime,news.n_imgsize,news.sign,news.advtype,news.isadv,news.n_url_type,news.n_content ';
		...
		//$rs=$this->cache("tongzhilgg",$this->cacheTime)->field($field)->alias('cls')->join("LEFT JOIN ios_news news ON  cls.id = news.cid")->where("cls.short_name='$sname' AND cls.isdel=0 AND news.isdel=0 AND news.isdisplay=1 AND news.itime<=NOW() AND news.offtime>NOW() $where")->order("$order news.orderid asc,news.utime desc,id DESC")->limit("$start,$lenght")->select();
		$rs=$this->cache(true,$this->cacheTime)->field($field)->alias('cls')->join("LEFT JOIN ios_news news ON  cls.id = news.cid")->where("cls.short_name='$sname' AND cls.isdel=0 AND news.isdel=0 AND news.isdisplay=1 AND news.itime<=NOW() AND news.offtime>NOW() $where")->order("$order news.orderid asc,news.utime desc,id DESC")->limit("$start,$lenght")->select();
