...
class MatchInfoModel extends Model{
	...
	public function getMatchList($where,$page=1,$lenght=10){
		...
		$sql = " select id,m_name,m_GameTime,m_untilTime,m_releaseTime,m_offineTime,m_city,m_Mtypes,m_ptype,m_sign,m_enterMode,m_placesleft,m_places,m_enterurl,m_state,m_img,m_order,m_country,m_area,m_signuptime,m_mtypes_class,m_des from {$this->trueTableName} where $where order by m_order desc,id desc $limit";
		return $this->cache(true,$this->cacheTime)->query($sql);
	}
	...
	public function search($where,$order){
		...
		$sql = " select id,m_name,m_GameTime,m_untilTime,m_releaseTime,m_offineTime,m_city,m_Mtypes,m_ptype,m_sign,m_enterMode,m_placesleft,m_places,m_enterurl,m_state,m_img,m_order,m_country,m_area,m_signuptime,m_mtypes_class,m_des from {$this->trueTableName} where $where order by $order m_order desc,id desc";
		return $this->cache(true,$this->cacheTime)->query($sql);
	}
	...
	public function searchV2($where,$order,$type){
		...
		switch($type){
			case 1:
				$initwhere .= " and m_sign_nuomi=1 ";
				$initorder = " m_order desc,m_GameTime asc,id desc ";
				break;
			case 2:
				$initwhere .= " and m_sign_nuomi=0 and m_placesleft>0 and m_untilTime>NOW() and m_GameTime>NOW() ";		
				break;
			case 3:
				$initwhere .= " and m_sign_nuomi=0 and (m_placesleft<=0 or m_untilTime<=NOW() or m_GameTime<NOW()) ";
