...
class OAuth
{
    ...
    public static function refreshToken()
    {
        $params = array(
            ...
            'refresh_token' => session('t_refresh_token')
...
//             $_SESSION['t_expire_in'] = $out['expires_in'];
        	session('t_access_token',$out['access_token']);
        	session('t_refresh_token',$out['refresh_token']);
        	session('t_expire_in',$out['expires_in']);
            return $out;
        } else {
            return $r;
        }
    ...
    {
        if ((session('t_access_token'))) (session('t_access_token',null));
        if ((session('t_expire_in'))) (session('t_expire_in',null));
        if ((session('t_code'))) (session('t_code',null));
        if ((session('t_openid'))) (session('t_openid',null));
        if ((session('t_openkey'))) (session('t_openkey',null));
        if ((session('t_oauth_version'))) (session('t_oauth_version',null));
    }
}
...
class Tencent
{
    ...
    {
        if (session('t_access_token')) {//OAuth 2.0 方式
            //鉴权参数
            ...
            $params['seqid'] = time();
            $params['serverip'] = session('SERVER_ADDR');
            
            $url = self::$apiUrlHttps.trim($command, '/');
        } elseif ((session('t_openid')) && (session('t_openkey'))) {//openid & openkey方式
    ...
    /**
     * 发起一个腾讯API请求
     * @param $data=array('t_access_token','t_openid') 接口名称 如：t/add
     * @param $command 接口名称 如：t/add
     * @param $params 接口参数  array('content'=>'test');
     * @param $method 请求方式 POST|GET
     * @param $multi 图片信息
     * @return string
     */
    public static function myapi($data=array(),$command, $params = array(), $method = 'GET', $multi = false)
    {
    	if(empty($data))return false;
    	if ($data['t_access_token']) {//OAuth 2.0 方式
    		//鉴权参数
    		$params['access_token'] = $data['t_access_token'];
    		$params['oauth_consumer_key'] = OAuth::$client_id;
    		$params['openid'] = $data['t_openid'];
    		$params['oauth_version'] = '2.a';
    		$params['clientip'] = Common::getClientIp();
    		$params['scope'] = 'all';
    		$params['appfrom'] = 'php-sdk2.0beta';
    		$params['seqid'] = time();
    		$params['serverip'] = $data['SERVER_ADDR'];
    
    		$url = self::$apiUrlHttps.trim($command, '/');
    	} elseif (($data['t_openid']) && ($data['t_openkey'])) {//openid & openkey方式
    		$params['appid'] = OAuth::$client_id;
    		$params['openid'] = $data['t_openid'];
    		$params['openkey'] = $data['t_openkey'];
    		$params['clientip'] = Common::getClientIp();
    		$params['reqtime'] = time();
    		$params['wbversion'] = '1';
    		$params['pf'] = 'php-sdk2.0beta';
