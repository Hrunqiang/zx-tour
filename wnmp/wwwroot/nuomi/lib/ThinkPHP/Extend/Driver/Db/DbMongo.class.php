...
class DbMongo extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            $config['username']	= $config['username']?$config['username']:$config['db_user'];
            $config['password']	= $config['password']?$config['password']:$config['db_pwd'];
            $config['hostname']	= $config['hostname']?$config['hostname']:$config['db_host'];
            $config['hostport']	= $config['hostport']?$config['hostport']:$config['db_port'];
            $config['database']	= $config['database']?$config['database']:$config['db_name'];
    ...
    public function group($keys,$initial,$reduce,$options=array(),$table=''){
    	if($table){
    		$this->switchCollection($table,'',false);
    	}
    ...
    public function parseWhere($where){
        ...
        foreach ($where as $key=>$val){
            if('_id' != $key && 0===strpos($key,'_') && 0!==strpos($key,'_id.')) {
                // 解析特殊条件表达式
    ...
    protected function parseThinkWhere($key,$val) {
        ...
        switch($key) {
            ...
            case '_exp':
            	parse_str($val,$query);
            	$query_s[$query['_k']]  = new MongoRegex($query['_v']);
            	$query	= $query_s;
            	break;
        }
        return $query;
