! function(a, b) {
	...
	function i(a) {
		...
		g = h({
			_t: +new Date
		}, g), n.onreadystatechange = function() {
	...
	k = function() {
		...
		return b = f("entry"), b && (j.entry = b), a = h(j),
			function(b) {
				return "/running/save_score?"
			}
	}(), b.ping = function(a, b) {
		var c = new Image;
		b && (a = h(b, a)), c.src = k(a)
	...
	b.isSupport = function() {
		...
		f && clearInterval(f), d(5000), f = setInterval(function() {
			d(5000)
		}, 1000)
	...
	function d(a) {
		this.startFrame = -1, this.frames = a, this.render = function(a, b) {
			...
			var j = h + 0.43 * e + g.wx,
				k = i - 0.052 * f;
			d.fillStyle = "#474540", d.beginPath(), d.moveTo(j, k), d.quadraticCurveTo(0.5 * e, k - 5, j + 0.13 * e, k), d.quadraticCurveTo(j + 0.3 * e, k + 0.1 * f, e, k + 0.4 * f), d.lineTo(e, f), d.lineTo(0, f), d.lineTo(0, k + 0.4 * f), d.quadraticCurveTo(j - 0.17 * e, k + 0.1 * f, j, k), d.closePath(), d.fill()
		}, this.reset = function() {
			this.startFrame = -1
	...
	d.canvas = e.$("#can-game"), d.FPS = f.FPS[0], d.frame = 0, d.timer = g.requestAnimationFrame || g.webkitRequestAnimationFrame || g.setTimeout, d.playing = !1, d.buffer = null, d.animateList = c.exports.animateList = [], c.exports.initCanvas = d.initCanvas = function() {
		...
		function a(f) {
			b = f || +new Date, e || (e = b), c.playing && (b - e > 1000 / d.FPS && (c.clearBuff(), c.renderBuff(), c.render(), ++c.frame, e = b), c.timer.call(g, a))
		}
		var b, c = d,
			e = 0;
	...
	c.exports.render = d.render = function(a, b) {
		...
		var b = d;
		b.ctx.fillStyle = a.color, b.ctx.beginPath(), b.ctx.arc(0.51 * b.w, 0.4 * b.h, a.r, 0, 2 * Math.PI), b.ctx.closePath(), b.ctx.fill()
	}
	...
	d.curGrade = 1, d.observer = a("widget/observer"), d.needCalcul = !1, d.gameTime = i.GAME_TIME, d.interval = 100, d.dis = 0, d.started = !1, d.showUpGrade = !1, c.exports.start = d.start = function() {
		e.once("game/setup", function() {
			...
			function a() {
				e.$("#page-game").removeEventListener("touchstart", a), j.started(), e.once("game/listen", function() {
					d.ListenDom()
				}), d.setupTimer(), h.animateList.push(n.sky), h.animateList.push(n.ground1), h.animateList.push(n.tree1), h.animateList.push(n.person1), h.play(), l.show(1, 1)
			}
			g.init([{
				q: "#g-ready",
				//hhy
		...
		this.observer.on("round", function() {
			...
			m.on(), d.observer.emit("log", {
				page: "music_turnon"
			})
		}), this.observer.on("musicoff", function() {
			m.off(), d.observer.emit("log", {
				page: "music_shutdown"
			})
		})
	...
	b.renderStart = function() {
		...
		e.render(l, a), new f(b).render(l, a), new g(b).render(l, a), j.render({
			...
			k.addClass(s, "sel"), t.innerHTML = "恭喜你跑了"
			console.log("游戏结束");
		}, 1000)
	}, b.reset = function() {
	...
	d.observer = a("widget/observer"), d.start = function() {
		...
		g.$("#i-start").addEventListener("click", function() {
			...
			showRule()
		}), g.$("#i-notice-download-close").addEventListener("click", function() {}), g.$("#i-reward").addEventListener("click", function() {
			d.getReward()
		})
	}, d.setupObserver = function() {
	...
	function c(a) {
		var b = {};
		return "friend" === a ? (b.title = k.SHAREFRIENDCONF.title.replace("xxx", g.getCurOwnDist()), b.url = k.SHAREFRIENDCONF.url, b.desc = k.SHAREFRIENDCONF.desc.replace("xxx", g.getCurOwnDist())) : b.title = k.SHARELINECONF.title.replace("xxx", g.getCurOwnDist(), b.url = k.SHARELINECONF.url), b.imgUrl = j.userInfo.avatar || window.default_avatar, b.link = n, b.success = function() {
			m.emit("log", {
				page: "click_share",
				step: 2,
				share_type: a,
	...
	b.share = function() {
		...
		e.$("#i-sharemask-uc").addEventListener("click", function() {
			e.$("#i-sharemask-uc").hide(), m.emit("log", {
				page: "click_share",
				step: 2,
				share_status: "cancel"
			})
	...
	function d() {
		this.curDis = 0, this.bestDis = 0, this.run = function(a, b) {
			...
			h.clearLocal(), h.setLocal(this.seq, {
	...
	b.record = function(a) {
		...
		return i.src = a.avatar || window.default_avatar, j.innerText = f.nick(a.nick), l.innerText = a.createTime, b = d.ownerInfo.userId === d.userInfo.userId ? e.i : e.he, m.innerText = a.runnerUserId === d.ownerInfo.userId ? f.dist1(a) : f.dist2(b, a), g.appendChild(h), g.appendChild(j), g.appendChild(k), h.appendChild(i), k.appendChild(l), k.appendChild(m), g
		...
		var f = c.createElement("li"),
			...
			l = parseInt((b + 1) / 4),
			m = e(b + 1);
		return f.appendChild(g), f.appendChild(h), f.appendChild(j), h.appendChild(i), f.style.top = 7 + 62 * l + "px", f.style.left = 29 + 74 * m + "px", 0 === a.type ? (i.src = a.avatar || window.default_avatar, j.innerText = a.dist + "m", g.className = "i-node-" + k) : 1 === a.type ? (h.className = "i-node-null", g.className = "i-node-" + k + "-g") : (h.className = "i-node-final", g.className = "i-node-" + k, d > 0 && (g.className += "-g")), f
	}
	...
	function d() {
		this.init = function() {
			...
			a.style.display = "none", a.style.position = "fixed", a.style.top = 0, a.style.left = 0, a.style.width = "100%", a.style.height = "100%", a.style.background = "rgba(0, 0, 0, .6)", a.style.zIndex = 1000, a.addEventListener("click", function() {
