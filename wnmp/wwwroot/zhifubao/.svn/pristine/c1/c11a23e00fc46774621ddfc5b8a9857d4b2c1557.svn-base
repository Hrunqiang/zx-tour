	...
	$.ajax({
		...
		success: function(data){
			if(data.error == 0 ){
				...
				var html = "<h2>"+data.data["firstname"]+" "+data.data["lastname"]+"</h2><div id='table'><table><tr><td>公里数</td><td>时间</td></tr>";
				for(var i in score){
					if(i=="Finish"){						
						html+="<tr><td class='f_t'>总成绩</td><td class='f_c'>"+score[i]+"</td></tr>";
					}else{
						out = score[i]['timeStr'];
						if(score[i]['name']=="Finish Net"){						
				...
				$("#wrap").html(html);
				$("#btn").click(function(){
					window.location.href = "/h5/score/2016boston/share.html";
				});
				// $("#img_btn").click(function(){
				// 	window.location.href = $(this).attr("img_url");
				// });
				shareurl = "http://weixin.zx-tour.com/h5/score/2016boston/?code="+code;
				sharetitle = "我的2016波士顿马拉松成绩是"+souceRes;
				wxbind();
			}else{
				alert("没有找到相关成绩！");
			}
