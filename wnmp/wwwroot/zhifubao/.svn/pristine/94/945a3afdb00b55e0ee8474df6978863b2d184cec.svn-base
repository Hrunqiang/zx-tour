...
class GoodsModel extends Model{
	...
	public function getwarmgoods($warmleft){
		if(empty($warmleft)) return false;
		$sql = "SELECT m.m_name,g.g_name,gg.g_name AS parent,g.g_stockleft FROM `zx_tb_goods` g  
...
AND g.g_state = 0 AND gg.g_state = 0 AND g.g_releaseTime <NOW() AND g.g_offineTime >NOW() AND g.g_isgoods=1 AND g.g_stockleft <=$warmleft";
		return $this->query($sql);
	}
