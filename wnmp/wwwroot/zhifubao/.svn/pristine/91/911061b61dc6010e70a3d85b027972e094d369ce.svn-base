...
/**
 * 发送邮件
 * @param 多个地址用英文,(逗号)隔开 $to
 * @param unknown_type $title
 * @param html $content
 * @param unknown_type $pic
 */
function send_email($to,$title,$content,$pic=''){
	if(!empty($to) && !empty($title) && !empty($content)){
		$mail = new SaeMail();
		$mail->setOpt(array('content_type'=>'HTML'));
		$mail->setAttach( array( 'my_photo' => $pic ) );
		$ret = $mail->quickSend( $to , $title , $content , 'mawentao5367519@163.com' , '850119gcj' );
