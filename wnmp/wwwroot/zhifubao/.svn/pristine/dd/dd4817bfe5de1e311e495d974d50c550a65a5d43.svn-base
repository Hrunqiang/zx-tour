...
//		echo $requestUrl;
// 		echo "**************请求url:**************<br>".$requestUrl."<br><pre>";
// 		echo "<br>**************请求的apiParams**************<br>";
//		var_dump($apiParams);
		//发起HTTP请求
		try {
			...
			if (!$this->checkEmpty($responseSubCode) || ( $this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				if (!$checkResult) {
					if (strpos($signData->signSourceData, "\\/") > 0) {
						...
						throw new Exception("check sign Fail1! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
					}
	...
	function parserJSONSignData($request, $responseContent, $responseJSON) {
		require_once dirname(__FILE__).'/SignData.php';
